  ...
  return (
    <div className="flex w-full flex-col">
      ...
      <section className="relative border-b border-(--border)">
        <div className="relative h-105 sm:h-130 overflow-hidden">
          <img
            src={article.image}
            alt={article.title}
          ...
          <div className="flex items-center justify-between gap-4">
            {prev ? (
              <div className="flex flex-col gap-1">
                ...
                <Button to={`/articles/${prev._id}`} variant="secondary">← {prev.title}</Button>
              </div>
            ) : <div />}
            {next ? (
              <div className="flex flex-col items-end gap-1">
                <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-(--muted)">Next</p>
                <Button to={`/articles/${next._id}`} variant="secondary">{next.title} →</Button>
              </div>
            ) : <div />}
          </div>
        </div>
